...
+ (void)addSearchIndexingToEntity:(NSEntityDescription *)entity onAttributes:(NSArray *)attributes
{
    ...
    NSEntityDescription *searchWordEntity = [entity.managedObjectModel entitiesByName][RKSearchWordEntityName];
    if (! searchWordEntity) {
    ...
    for (id attributeIdentifier in attributes) {
        ...
        if ([attributeIdentifier isKindOfClass:[NSString class]] ||
            ...
            [attributeIdentifier isKindOfClass:[NSSet class]] ||
            [attributeIdentifier isKindOfClass:[NSNumber class]]) {
            // Look it up by name
            attribute = [entity attributesByName][attributeIdentifier];
            ...
            @throw [NSException exceptionWithName:NSInvalidArgumentException
                                           reason:[NSString stringWithFormat:@"Unable to configure search indexing: Invalid attribute identifier of type '%@' given, expected an NSString, NSNumber, NSArray, NSSet or NSAttributeDescription. (Value: %@)", [attributeIdentifier class], attributeIdentifier]
                                         userInfo:nil];
        }

        NSAssert(attribute.attributeType == NSStringAttributeType || attribute.attributeType == NSTransformableAttributeType || (attribute.attributeType >= NSInteger16AttributeType && attribute.attributeType <= NSFloatAttributeType), @"Invalid attribute identifier given: Expected an attribute of type NSStringAttributeType, NSTransformableAttributeType, NSInteger16AttributeType, NSInteger32AttributeType, NSInteger64AttributeType, NSDecimalAttributeType, NSDoubleAttributeType, NSFloatAttributeType got %ld.", (unsigned long) attribute.attributeType);
    ...
    userInfo[RKSearchableAttributeNamesUserInfoKey] = attributeNames;
...
- (NSUInteger)indexManagedObject:(NSManagedObject *)managedObject withProgressBlock:(void (^)(NSManagedObject *managedObject, RKSearchWord *searchWord, BOOL *stop))progressBlock;
{
    @autoreleasepool {
        ...
        NSPredicate *predicateTemplate = [NSPredicate predicateWithFormat:@"%K IN $CURRENT_OBJECT_TOKENS", RKSearchWordTokenAttributeName];
        NSManagedObjectContext *managedObjectContext = managedObject.managedObjectContext;
        __block BOOL stop = NO;
        
        // TODO: [de] This method has been refactored for performance and used locale. Needs refactoring.

        // the app locale currently in use
        NSLocale *usedLocale = [[NSLocale alloc] initWithLocaleIdentifier:[[[NSBundle mainBundle] preferredLocalizations] objectAtIndex:0]];
        
        [managedObjectContext performBlockAndWait:^{

            // store tokens and original words
            ...
            for (NSString *searchableAttribute in searchableAttributes) {
                ...
                for (id object in collectionOfStrings) {
                    ...
                    if (attributeValue) {
                        ...
                        NSSet *tokens = [searchTokenizer tokenize:attributeValue forLocale:usedLocale foldString:NO];
                        for (NSString *token in tokens) {
                            NSString *foldedToken = [token stringByFoldingWithOptions:kCFCompareCaseInsensitive|kCFCompareDiacriticInsensitive locale:usedLocale ];
                            tokenDictionary[foldedToken] = token; // TODO might need a collection here
                        }
                    }
                }
            }
            NSMutableSet *foldedTokens = [NSMutableSet setWithArray:[tokenDictionary allKeys]];

            // Create and perform a request for existing keyword records for these tokens
            NSError *error;
            fetchRequest.predicate = [predicateTemplate predicateWithSubstitutionVariables:@{@"CURRENT_OBJECT_TOKENS": foldedTokens}];
            NSMutableSet* searchWords = [NSMutableSet setWithArray:[managedObjectContext executeFetchRequest:fetchRequest error:&error]];

            if (!error) {
                // Find nonexistent keywords (by removing existing keywords from tokens) ...
                [foldedTokens minusSet:[searchWords valueForKey:RKSearchWordTokenAttributeName]];
                
                // ... then create them
                for (id foldedToken in foldedTokens)
                {
                    NSString *originalToken = tokenDictionary[foldedToken];
                    
                    RKSearchWord *searchWord = [NSEntityDescription insertNewObjectForEntityForName:RKSearchWordEntityName inManagedObjectContext:managedObjectContext];
                    [searchWord setValue:foldedToken forKey:RKSearchWordTokenAttributeName];
                    [searchWord setValue:originalToken forKey:RKSearchWordOriginalWordAttributeName];
                    ...
                    if ([self.delegate respondsToSelector:@selector(searchIndexer:didInsertSearchWord:forWord:inManagedObjectContext:)]) {
                        [self.delegate searchIndexer:self didInsertSearchWord:searchWord forWord:foldedToken inManagedObjectContext:managedObjectContext];
                    }
                ...
                if (! stop) {
                    ...
                    RKLogTrace(@"Indexed search words: %@", [searchWords valueForKey:RKSearchWordTokenAttributeName]);
                ...
                RKLogError(@"Failed to retrieve search words: %@", error);
            }
        }];
...
- (NSSet *)objectsToIndexFromCandidateObjects:(NSSet *)objects checkChangedValues:(BOOL)checkChangedValues
{
    ...
    for (NSManagedObject *managedObject in objects) {        
        ...
        for (NSString *attribute in searchableAttributes) {
            if (!checkChangedValues || [managedObject changedValues][attribute]) {
